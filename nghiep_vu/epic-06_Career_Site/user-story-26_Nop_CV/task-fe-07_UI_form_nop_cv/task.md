# task-fe-07_UI_form_nop_cv

## Mục đích
Xác định phạm vi frontend cho task 'UI form nop cv' trong US-26 Nop CV, làm rõ task dùng để làm gì và liên kết với luồng nghiệp vụ tương ứng.
...
- Form hiện gửi multipart với các part `fullName`, `email`, `phone`, `coverLetter`, `cvFile`, `answers` và `consentAccepted`.
- UI kiểm tra nhanh PDF tối đa 5MB, hiển thị loading/error/duplicate và trang xác nhận có tracking token; backend vẫn là nguồn chuẩn.
- Custom field loại `FILE` chưa có part upload riêng trong contract hiện tại; chỉ các câu trả lời text/URL/SELECT được gửi trong `answers`.
