# task-fe-03_UI_quan_ly_trang_thai_dang_nhap

## Mục đích
Task này dùng để xử lý nghiệp vụ liên quan đến chức năng task-fe-03 UI quan ly trang thai dang nhap.

## Mô tả chức năng chi tiết
Khởi tạo Global State (sử dụng React Context hoặc Redux Toolkit) để lưu trữ thông tin `user` và `isAuthenticated`. Viết các reducer/action xử lý quá trình Login, Logout và tự động gọi API Refresh Token khi Access Token hết hạn.

## Mục đích
Xây dựng screen/component phục vụ US-01 - HR đăng nhập, tập trung vào hành vi người dùng và trạng thái UI.

## Screen/Component
- Component chính: AuthContext.
